...
| BMAD lié | `À créer après validation du PRD` |
| ADR liés | `Aucun requis à ce stade` |
| Tickets liés | `À créer depuis le premier dossier BMAD` |
...
2. créer `BMAD-PRIMIE-001` et ses tickets `READY` ;
...
| `G1 — PRD` | PRD relu et explicitement validé | `En attente` |
| `G2 — Solution` | architecture et risques acceptés | `En attente` |
| `G3 — Ready` | tickets BMAD exécutables | `Bloqué par G1` |
...
- [ ] le CTO valide le périmètre et la stack cible ;
- [ ] les inconnues bloquantes sont assignées ;
- [ ] la maquette de référence est identifiée ;
- [ ] les assets indispensables au premier incrément sont disponibles ;
- [ ] `BMAD-PRIMIE-001` est créé ;
...
- [ ] vision, structure, prestations et périmètre validés ;
- [ ] parcours WhatsApp et inconnues métier acceptés ;
- [ ] maquette, assets et droits identifiés ;
- [ ] stack et architecture validées ;
- [ ] accessibilité, performance, SEO et sécurité acceptés ;
- [ ] dépendances assignées ;
...
### Statut actuel
`Proposé — en attente de validation G1`.

### Actions suivantes
1. faire relire les sections métier par Prisca ;
2. obtenir le GO du CTO sur le périmètre ;
3. passer le document en version `1.0 — Validé` ;
4. créer `BMAD-PRIMIE-001` ;
5. ne commencer le code qu’après des tickets `READY`.
